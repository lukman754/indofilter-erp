        ...
        <template v-else>
            ...
            <div class="space-y-4">
                ...
                <div class="bg-white rounded-lg border border-gray-200 p-6 space-y-4">
                    ...
                    <div class="space-y-3">
                        <div>
                            <label class="block text-xs font-medium text-gray-655 mb-1 font-semibold">
                            ...
                            <div class="bg-blue-50/50 border border-blue-100 rounded-lg p-3 text-xs text-blue-800 space-y-1 mt-2">
                                ...
                                <p class="text-[11px] leading-relaxed">
                                    Jika Anda mengisi **Base Path Utama** (contoh: <code class="bg-blue-100 px-1 py-0.5 rounded font-mono font-bold text-[10px]">G:\My Drive\Arthawa</code>), 
                                    sistem akan secara otomatis menyusun dan membuat folder penyimpanan dokumen di komputer Anda dengan struktur:
                                    <br />
                                    <code class="bg-blue-100 px-1 py-0.5 rounded font-mono font-bold text-[10px] select-all">[Base Path] \ [Nama Perusahaan] \ [TIPE DOKUMEN]</code>
                                    <br />
                                    Misal untuk Invoice pada PT. INDO FILTER SEMESTA:
                                    <br />
                                    <code class="bg-blue-100 px-1 py-0.5 rounded font-mono font-bold text-[10px] select-all">G:\My Drive\Arthawa\PT INDO FILTER SEMESTA\INVOICE</code>
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="bg-white rounded-lg border border-gray-200 p-6 space-y-4">
                    ...
                    <div class="bg-purple-50/50 border border-purple-100 rounded-lg p-3 text-xs text-purple-950 space-y-1">
                        <p class="font-bold flex items-center gap-1">
                            💡 Petunjuk Penggunaan Gemini API:
                        </p>
                        <p class="text-[11px] leading-relaxed">
                            API Key digunakan untuk fitur pembacaan PDF otomatis menggunakan kecerdasan buatan. 
                            Anda dapat membuat API Key secara gratis di <a href="https://aistudio.google.com/" target="_blank" class="text-purple-700 hover:text-purple-900 underline font-bold transition-colors">Google AI Studio</a>. 
                            Jika kolom ini dikosongkan, sistem akan mencoba menggunakan konfigurasi <code class="bg-purple-100 px-1 py-0.5 rounded font-mono font-bold text-[10px]">GEMINI_API_KEY</code> dari file .env.
                            Sedangkan Model Gemini menentukan model mana yang akan digunakan untuk pemrosesan dokumen (bawaan: <code class="bg-purple-100 px-1 py-0.5 rounded font-mono font-bold text-[10px]">gemini-1.5-flash</code>).
                        </p>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-lg border border-gray-200 p-6 space-y-4">
                    ...
                    <div class="bg-amber-50/50 border border-amber-200 rounded-lg p-3 text-xs text-amber-800">
                        <p class="font-bold flex items-center gap-1 mb-1">
                            ⚠️ Informasi Pemetaan Manual:
                        </p>
                        Jika kolom-kolom path di bawah ini diisi, sistem akan menggunakan path absolut yang Anda masukkan secara manual untuk menimpa aturan default di atas. 
                        <strong>Biarkan kosong</strong> jika ingin menggunakan aturan subfolder otomatis.
                    </div>

                    <!-- Global Folders form -->
                    <div
                        v-if="activeCompanyTab === 'global' && form.global_folders"
                        class="grid grid-cols-1 md:grid-cols-2 gap-4"
                    >
                        <div v-for="dt in documentTypes" :key="dt.key">
                            <label class="block text-xs font-medium text-gray-655 mb-1">
                                {{ dt.label }}
                            </label>
                            <input
                                v-model="form.global_folders[dt.key]"
                                type="text"
                                :placeholder="'Contoh: G:\\My Drive\\Arthawa\\Global\\' + dt.key.toUpperCase()"
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg text-xs focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
                            />
                        </div>
                    </div>
                    ...
                    >
                        <div v-for="dt in documentTypes" :key="dt.key">
                            ...
                            <input
                                ...
                                type="text"
                                :placeholder="'Contoh: G:\\My Drive\\Arthawa\\Khusus\\' + dt.key.toUpperCase()"
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg text-xs focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
                            />
                        </div>
                    </div>
