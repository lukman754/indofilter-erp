...
async function handleConfirm() {
    ...
    try {
        await api.confirm(route.params.id)
        await loadDocument()
    } catch (e) {
...
async function handleExport() {
    try {
        const filename = doc.value && doc.value.number ? `${doc.value.number.replace(/[\/\\]/g, '-')}.docx` : `dokumen-${route.params.id}.docx`
        const res = await api.export(route.params.id)
...
const syncStorageLoading = ref(false)

async function handleSyncStorage() {
    syncStorageLoading.value = true
    try {
        ...
        syncStorageLoading.value = false
    }
}
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="divide-y divide-gray-200">
                                <tr v-for="(item, i) in doc.items" :key="i" :class="i % 2 === 0 ? 'bg-white' : 'bg-gray-50'">
                                    ...
                                    <td v-if="doc.type !== 'delivery_slip'" class="px-3 py-2 text-sm text-gray-700 text-right">{{ formatMoney(item.unit_price) }}</td>
                                    <td v-if="doc.type !== 'delivery_slip'" class="px-3 py-2 text-sm text-gray-700 text-right font-medium">{{ formatMoney(item.total) }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
