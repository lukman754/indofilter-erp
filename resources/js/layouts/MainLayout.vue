    ...
    if (e.ctrlKey && (e.key === "ArrowDown" || e.key === "ArrowUp")) {
        ...
        if (activeIndex !== -1) {
            if (e.key === "ArrowDown") {
                ...
                nextIndex =
                    (activeIndex - 1 + sidebarPaths.length) %
                    sidebarPaths.length;
            }
        }
    ...
    <div class="flex h-screen bg-odoo-gray-bg">
        ...
        <template v-else>
            ...
            <div class="flex-1 flex flex-col overflow-hidden">
                ...
                />
                <main
                    class="flex-1 overflow-y-auto p-4 pt-0 md:p-6 md:pt-0 bg-odoo-gray-bg"
                >
                    <router-view />
                </main>
            </div>
        ...
        <Teleport to="body">
            <Transition name="modal">
                <div
                    v-if="appStore.confirmModal.show"
                ...
                >
                    ...
                    ></div>
                    <div
                        class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md transition-all overflow-hidden border border-gray-100"
                    >
                        <!-- Icon & Content -->
                        <div class="p-6 flex flex-col items-center text-center">
                            <!-- Success Icon -->
                            <div
                                v-if="appStore.confirmModal.type === 'success'"
                                class="w-12 h-12 bg-green-50 text-green-600 rounded-full flex items-center justify-center mb-4"
                            >
                                <svg
                                    class="w-6 h-6"
                                    fill="none"
                            ...
                            <div
                                v-else-if="
                                    appStore.confirmModal.type === 'error'
                                "
                            ...
                            <div
                                v-else
                                class="w-12 h-12 bg-amber-50 text-amber-600 rounded-full flex items-center justify-center mb-4"
                            >
                                <svg
                                    class="w-6 h-6"
                                    fill="none"
                            ...
                            <h3 class="text-lg font-bold text-gray-800 mb-2">
                                {{ appStore.confirmModal.title }}
                            </h3>
                            <p
                                class="text-sm text-gray-500 whitespace-pre-wrap leading-relaxed"
                            >
                                {{ appStore.confirmModal.message }}
                            </p>
                        </div>
                            ...
                            <button
                                ...
                                :class="
                                    appStore.confirmModal.type === 'error'
                                        ...
                                        : appStore.confirmModal.type ===
                                            'success'
                                          ? 'bg-green-600 hover:bg-green-700'
                                          : 'bg-indofilter hover:bg-indofilter-dark'
                                "
                            >
...
@media print {
    ...
    /* Paksa container agar tidak men-scroll (overflow) saat print, sehingga semua row tabel tercetak */
    .overflow-y-auto,
    .overflow-hidden,
    .flex-1,
    main,
